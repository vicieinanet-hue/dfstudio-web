...
async function initAudio() {
    try {
        ...
        try {
            ...
            logMsg('Microfone OK!');
        } catch(micErr) { logMsg('Microfone indisponivel: ' + micErr.message); }
    ...
    } catch(e) { logMsg('ERRO: ' + e.message); showError('Toque no botao para tentar novamente'); }
}
...
function toggleSolo(ch) {
    ...
    if(btn) { var active = (ch==='voice')?isMicSolo:isPlaySolo; if(active) btn.classList.add('solo-active'); else btn.classList.remove('solo-active'); }
}
...
function toggleMonitor(){ monitorEnabled=!monitorEnabled; setMonGain(monGain); var btn=document.getElementById('monitorBtn'); if(btn){if(monitorEnabled)btn.classList.add('active');else btn.classList.remove('active');} }
