...
var micMuted = true, playMuted = true, masterMuted = true;
...
async function start() {
    try {
        ...
        micG = ctx.createGain(); micG.gain.value = 0;
        playG = ctx.createGain(); playG.gain.value = 0;
        masterG = ctx.createGain(); masterG.gain.value = 0;
        monG = ctx.createGain(); monG.gain.value = 0;
        ...
        micG.connect(aMic); aMic.connect(monG); monG.connect(masterG);
        playG.connect(aPlay); aPlay.connect(masterG);
        masterG.connect(aMasterL); masterG.connect(aMasterR);
        aMasterL.connect(ctx.destination); aMasterR.connect(ctx.destination);
        log('Microfone...');
        try {
            var s = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: false, noiseSuppression: false, autoGainControl: false } });
            ctx.createMediaStreamSource(s).connect(micG);
            log('Microfone OK!');
        } catch (e) { log('Microfone indisponivel'); }
        document.getElementById('sr').textContent = (ctx.sampleRate / 1000).toFixed(1) + 'k';
        masterMuted = false; masterG.gain.value = masterGain;
        document.getElementById('masterMute').classList.remove('mute-on');
        document.getElementById('splash').style.display = 'none';
        document.getElementById('app').style.display = 'flex';
        loadMarkers(); buildKeys(); buildTunerStrings();
        setInterval(loop, 80);
        log('✅ PRONTO!');
    } catch (e) { log('ERRO: ' + e.message); err('Toque para tentar'); }
}
...
function vu(a) { if (!a) return 0; a.getByteTimeDomainData(vuData); var s = 0; for (var i = 0; i < vuData.length; i++)s += Math.abs(vuData[i] - 128); return Math.min(1, s / vuData.length / 128 * 1.5); }
function fm(s) { var m = Math.floor(s / 60), sec = Math.floor(s % 60); return m.toString().padStart(2, '0') + ':' + sec.toString().padStart(2, '0'); }

function setMic(v) { micGain = v; if (micG) micG.gain.value = micMuted ? 0 : v * 4; }
function setPlay(v) { playGain = v; if (playG) playG.gain.value = playMuted ? 0 : v; }
function setMaster(v) { masterGain = v; if (masterG) masterG.gain.value = masterMuted ? 0 : v; }
function setMon(v) { monGain = v; if (monG) monG.gain.value = monOn ? v : 0; }

function toggleMute(ch) {
    if (ch === 'voice') { micMuted = !micMuted; setMic(micGain); var b = document.getElementById('voiceMute'); if (b) { if (micMuted) b.classList.add('mute-on'); else b.classList.remove('mute-on'); } }
    else if (ch === 'play') { playMuted = !playMuted; setPlay(playGain); var b = document.getElementById('playMute'); if (b) { if (playMuted) b.classList.add('mute-on'); else b.classList.remove('mute-on'); } }
    else { masterMuted = !masterMuted; setMaster(masterGain); var b = document.getElementById('masterMute'); if (b) { if (masterMuted) b.classList.add('mute-on'); else b.classList.remove('mute-on'); } }
}
function toggleSolo(ch) { if (ch === 'voice') micSolo = !micSolo; else playSolo = !playSolo; var b = document.getElementById(ch === 'voice' ? 'voiceSolo' : 'playSolo'); if (b) { var a = ch === 'voice' ? micSolo : playSolo; if (a) b.classList.add('solo-on'); else b.classList.remove('solo-on'); } }
function toggleMono() { playMono = !playMono; var b = document.getElementById('monoOn'); if (b) { if (playMono) b.classList.add('on'); else b.classList.remove('on'); } }
function toggleMon() { monOn = !monOn; setMon(monGain); var b = document.getElementById('monSw'); if (b) { if (monOn) b.classList.add('on'); else b.classList.remove('on'); } document.getElementById('monLabel').textContent = monOn ? 'RETORNO APP' : 'DIRECT MON'; }
function toggleFx() { var b = document.getElementById('fxOn'); b.classList.toggle('on'); }
function toggleAt() { var b = document.getElementById('atOn'); b.classList.toggle('on'); }
function setPan(ch, v) { }
function resetMixer() { setMic(.65); setPlay(.40); setMaster(.50); setMon(.50); document.getElementById('micGain').value = 65; document.getElementById('playGain').value = 40; document.getElementById('masterFader').value = 50; document.getElementById('voiceFader').value = 50; document.getElementById('playFader').value = 50; document.getElementById('monGain').value = 50; }

function togglePlay() { if (playing) { try { srcNode.stop(); } catch (e) { } playing = false; } else if (audioBuf) { playFrom(pos); } document.getElementById('playBtn').textContent = playing ? '⏸️' : '▶️'; }
function stopTrack() { try { srcNode.stop(); } catch (e) { } playing = false; pos = 0; document.getElementById('playBtn').textContent = '▶️'; }
function playFrom(p) { if (!audioBuf || !ctx) return; try { srcNode.stop(); } catch (e) { } srcNode = ctx.createBufferSource(); srcNode.buffer = audioBuf; srcNode.connect(playG); srcNode.start(0, p || 0); playing = true; pos = p || 0; startTime = ctx.currentTime - (p || 0); document.getElementById('playBtn').textContent = '⏸️'; }
async function loadFile(e) { var f = e.target.files[0]; if (f) { try { var b = await f.arrayBuffer(); audioBuf = await ctx.decodeAudioData(b); document.getElementById('trackName').textContent = '🎵 ' + f.name.replace(/\.[^/.]+$/, ''); document.getElementById('durTime').textContent = fm(audioBuf.duration); libraryFiles.push(f); updateLibList(); } catch (ex) { } } }
...
function saveMarker() {
    var n = document.getElementById('markerName').value || 'Marcador'; var tu;
    if (document.getElementById('useCurTime').checked) { tu = (ctx.currentTime - startTime) * 1000000; }
    else { var min = parseInt(document.getElementById('mm').value) || 0; var sec = parseInt(document.getElementById('ss').value) || 0; var ms = parseInt(document.getElementById('ms').value) || 0; tu = (min * 60 + sec) * 1000000 + ms * 1000; }
    markers.push({ name: n, timeUs: tu }); markers.sort(function (a, b) { return a.timeUs - b.timeUs; }); saveMarkers(); updMarkerBadge(); closeDlg();
}
function prevMarker() { if (!markers.length) return; var cur = (ctx.currentTime - startTime) * 1000000; var p = markers.filter(function (m) { return m.timeUs < cur - 500000; }).pop() || markers[markers.length - 1]; if (p) playFrom(p.timeUs / 1000000); }
function nextMarker() { if (!markers.length) return; var cur = (ctx.currentTime - startTime) * 1000000; var n = markers.find(function (m) { return m.timeUs > cur + 500000; }) || markers[0]; if (n) playFrom(n.timeUs / 1000000); }

function exportMarkers() {
    if (!markers.length) { alert('Nenhum marcador'); return; }
    var txt = 'MARCADORES\n----------\n';
    markers.forEach(function (m) { var t = m.timeUs / 1000, h = Math.floor(t / 3600000), min = Math.floor((t % 3600000) / 60000), s = Math.floor((t % 60000) / 1000), ms = Math.floor(t % 1000); var ts = (h > 0 ? h.toString().padStart(2, '0') + ':' : '') + min.toString().padStart(2, '0') + ':' + s.toString().padStart(2, '0') + '.' + ms.toString().padStart(3, '0'); txt += ts + ' | ' + m.name + '\n'; });
    var b = new Blob([txt], { type: 'text/plain' }); var a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = 'marcadores.txt'; a.click();
}
function importMarkers(e) {
    var f = e.target.files[0]; if (!f) return; var r = new FileReader();
    r.onload = function () {
        var lines = r.result.split('\n'); var newMarkers = [];
        lines.forEach(function (l) { if (!l.includes('|')) return; var p = l.split('|'); if (p.length < 2) return; var ts = p[0].trim(), nm = p[1].trim(); var tp = ts.split(/[:.]/); var h = 0, min = 0, s = 0, ms = 0; if (tp.length >= 4) { h = parseInt(tp[0]) || 0; min = parseInt(tp[1]) || 0; s = parseInt(tp[2]) || 0; ms = parseInt(tp[3]) || 0; } else if (tp.length >= 3) { min = parseInt(tp[0]) || 0; s = parseInt(tp[1]) || 0; ms = parseInt(tp[2]) || 0; } var tu = (h * 3600000 + min * 60000 + s * 1000 + ms) * 1000; newMarkers.push({ name: nm, timeUs: tu }); });
...
function loadLibFile(i) { var f = libraryFiles[i]; var reader = new FileReader(); reader.onload = function (e) { var b = e.target.result; ctx.decodeAudioData(b, function (buf) { audioBuf = buf; document.getElementById('trackName').textContent = '🎵 ' + f.name.replace(/\.[^/.]+$/, ''); document.getElementById('durTime').textContent = fm(audioBuf.duration); document.getElementById('mute2').classList.remove('mute-on'); playMuted = false; setPlay(playGain); }); }; reader.readAsArrayBuffer(f); }

// NAVEGAÇÃO
function showTab(t) {
    document.getElementById('tabMixer').classList.remove('active');
    document.getElementById('tabEffects').classList.remove('active');
    document.getElementById('tabAutotune').classList.remove('active');
    document.getElementById('tabTuner').classList.remove('active');
    ...
    var bs = document.querySelectorAll('.nav button'); bs.forEach(function (b, i) { b.classList.remove('active'); if ((t === 'mixer' && i === 0) || (t === 'effects' && i === 1) || (t === 'autotune' && i === 2) || (t === 'library' && i === 3) || (t === 'settings' && i === 4)) b.classList.add('active'); });
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('seekBar').addEventListener('input', function (e) { if (audioBuf) playFrom((e.target.value / 1000) * audioBuf.duration); });
    setTimeout(function () { start().catch(function () { err('Toque para iniciar'); }); }, 800);
});
